...
export class SessionManager {
    ...
    createSession(projectRoot, modelConfig) {
        ...
        this.db.prepare('INSERT INTO sessions (id, project_root, status, model_config, state, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?)').run(id, projectRoot, 'active', JSON.stringify(modelConfig), JSON.stringify(state), now, now);
        return session;
    }
    getSession(id) {
    ...
    updateSession(id, updates) {
        ...
        this.db.prepare('UPDATE sessions SET status = ?, state = ?, updated_at = ? WHERE id = ?').run(status, JSON.stringify(state), Date.now(), id);
        return true;
    }
    pauseSession(id) {
    ...
    addMessage(message) {
        ...
        this.db.prepare('INSERT INTO messages (id, session_id, role, content, tool_call, tool_result, timestamp, checkpoint_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(id, message.sessionId, message.role, message.content, message.toolCall ? JSON.stringify(message.toolCall) : null, message.toolResult ? JSON.stringify(message.toolResult) : null, message.timestamp, message.checkpointId || null);
        // Update session updatedAt
        this.db.prepare('UPDATE sessions SET updated_at = ? WHERE id = ?').run(Date.now(), message.sessionId);
        return msg;
    }
    getHistory(sessionId, limit = 100) {
        const rows = this.db.prepare('SELECT * FROM messages WHERE session_id = ? ORDER BY timestamp DESC LIMIT ?').all(sessionId, limit);
