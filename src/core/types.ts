...
export interface Checkpoint {
  ...
  iteration: number;
  description: string;
  fileSnapshots: Record<string, string>; // path → content
  createdAt: number;
}
...
export type Event =
  ...
  | { type: 'lsp_diagnostic'; file: string; diagnostics: LSPDiagnostic[] }
  | { type: 'tool_executed'; tool: string; result: ToolResult }
  | { type: 'test_completed'; passed: number; failed: number; duration: number }
  | { type: 'compliance_checked'; violations: ComplianceViolation[]; riskScore: number }
  | { type: 'checkpoint_created'; id: string; sessionId: string }
  ...
  | { type: 'agent_error'; error: string; iteration: number }
  | { type: 'hook_triggered'; hookName: string; triggerEvent: string }
  | { type: 'commission_validated'; isValid: boolean; discrepancies: string[] }
  | { type: 'license_checked'; agentId: string; status: LicenseStatus };
